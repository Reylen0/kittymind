...
class EventBus:
    ...
    async def emit(self, event_type: str, data: dict | None = None) -> None:
        ...
        异常被静默捕获（return_exceptions=True），不会中断其他处理器。
        """
        data = data or {}
        tasks = [
            h(event_type, data) for h in self._handlers.get(event_type, [])
        ]
        tasks += [
            h(event_type, data) for h in self._handlers.get("*", [])
        ]
        if tasks:
