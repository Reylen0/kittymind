...
3. **批内无依赖**：同一批返回的工具调用可能被并行执行，因此一批之内不得有依赖关系。\
互不依赖的只读操作（如同时检索多个来源、批量读取文件）鼓励合批以提高效率；\
有依赖的调用必须把前置工具单独作为一批先返回，确认结果后再发后续调用；\
遇到歧义先询问
4. **谨慎破坏性操作**：删除文件、rm -rf 等操作前主动告知用户
5. **优先工具**：对文件的操作用工具完成，不要在回复中输出大段代码替代工具调用
6. **验证优先**：修改代码/配置或启动服务后，用 verify 工具确认真的达到预期\
（跑测试/lint 或探测服务就绪），不要假设操作成功；验证失败要先尝试修复再重新验证\
"""
...
3. **批内无依赖**：同一批返回的工具调用可能被并行执行，一批之内不得有依赖关系；\
互不依赖的只读操作可以合批，有依赖的调用把前置工具单独作为一批先返回，\
确认结果后再发后续调用
4. **精确替换**：file_edit 的 old_string 要包含足够上下文
5. **只输出结论**：完成后用简洁文字总结结果，不要输出中间过程
6. **验证优先**：修改代码后如有 verify 工具，用它确认真的达到预期，不要假设操作成功\
"""
...
MEMORY_CONSOLIDATE = (
    "请合并重复的、删除过时的记忆，并为合并项写一条新的描述。\n"
    "只输出一个 JSON 数组，每个元素是一条操作，格式三选一：\n"
    '{"op": "keep", "name": "<保留的记忆名>"}\n'
    '{"op": "merge", "into": "<保留的记忆名>", "from": ["<被并入的记忆名>", "..."], '
    '"description": "<合并后的一行描述，80字以内>"}\n'
    '{"op": "drop", "name": "<已过时、应删除的记忆名>"}\n'
    "\n"
    "规则：\n"
    "1. name / into / from 必须与输入中的记忆名完全一致（照抄，不要翻译、改写或缩写）。\n"
    "2. 不要输出 body 字段，正文由程序自动合并，你只需决定「谁并到谁」。\n"
    "3. 没有变化的记忆不用写进数组，程序会原样保留。\n"
    "4. 描述中不要使用英文双引号，需要时用中文引号。\n"
    "5. 只返回 JSON 数组，不要输出任何其他内容。"
)
