...
class SessionManager:
    ...
    def list_sessions(self) -> list[dict]:
        ...
        for sid in self.store.list_ids():
            ...
            if header:
                result.append({"id": header["id"], "title": header.get("title", ""), "created_at": header.get("created_at", "")})
    ...
    def append_turn(self, session_id: str, turn_messages: list[dict], first_input: str | None = None) -> None:
        if not self.store.exists(session_id):
            self.create_session(title=self.generate_title(first_input or ""), session_id=session_id)
