...
曾经的问题（2026-09-17 修复）：全进程只有一份 (loop, push_fn)，
  - 连接 B 上线 → 覆盖 A 的推送目标 → A 触发的审批弹到 B 的窗口，A 等到超时被拒；
  - 任一连接断开 → clear_connection() 把**所有**挂起请求 set 成 deny 并清空推送目标
    → 另一个仍活跃的连接之后每次审批都被静默拒绝；
  - 还曾把 A 的 pending 一并清掉，A 的 worker 线程被误唤醒。
