...
def test_exact_failure_warn(ctrl):
    args = {"command": "cat /nope"}
    for i in range(1, 4):
        dec = ctrl.after_call("bash", args, "Error: no such file", True)
        if i < 2:
...
def test_exact_failure_no_block_in_interactive(ctrl):
...
def test_exact_failure_block_in_hard_stop(hard_ctrl):
    """非交互态：超 block 阈值时 before_call 应 block（任意 block code）。"""
    args = {"command": "cat /nope"}
    for _ in range(5):
        hard_ctrl.after_call("bash", args, "Error: no such file", True)
    dec = hard_ctrl.before_call("bash", args)
    assert dec.is_block


# ── GuardrailController — 同工具不同参失败 warn ─────────────────────

def test_same_tool_failure_warn(ctrl):
    for i in range(1, 5):
        dec = ctrl.after_call("bash", {"command": f"cat /nope{i}"}, "Error: fail", True)
        if i < 3:
...
def test_no_progress_warn_idempotent(ctrl):
    args = {"path": "/tmp/file.txt"}
    result = "hello"
    for i in range(1, 4):
        dec = ctrl.after_call("file_read", args, result, False)
        if i < 2:
...
def test_no_progress_not_triggered_for_mutating(ctrl):
    """变更工具（bash）即使返回相同结果也不触发无进展。"""
    for _ in range(5):
        dec = ctrl.after_call("bash", {"command": "echo hi"}, "hi", False)
    assert dec.action == "allow"

def test_no_progress_different_results_resets(ctrl):
    """幂等工具返回不同结果时计数重置。"""
    for r in ["a", "b", "a", "a"]:
        dec = ctrl.after_call("file_read", {"path": "x"}, r, False)
...
def test_identical_streak_block_interactive(ctrl):
    """交互态下连续 3 次相同 (sig, result) 也会 block。"""
    args = {"path": "x"}
    result = "same"
    # 第 1、2 次不触发（count=1,2）
    for _ in range(2):
        ctrl.after_call("file_read", args, result, False)
        dec = ctrl.before_call("file_read", args)
...
def test_identical_streak_reset_on_different_result(ctrl):
    """不同结果打断连续流，重置计数。"""
    for _ in range(3):
        ctrl.after_call("file_read", {"path": "x"}, "same", False)
    ctrl.after_call("file_read", {"path": "x"}, "different", False)
    dec = ctrl.before_call("file_read", {"path": "x"})
    assert not dec.is_block

def test_identical_streak_reset_on_different_args(ctrl):
    """不同参数打断连续流。"""
    for _ in range(3):
        ctrl.after_call("file_read", {"path": "x"}, "r", False)
    ctrl.after_call("file_read", {"path": "y"}, "r", False)
    dec = ctrl.before_call("file_read", {"path": "x"})
    assert not dec.is_block
...
def test_progress_resets_exact_failure(ctrl):
    """变更类工具成功后，相同 sig 的失败计数视为新实验。"""
    args = {"command": "cat /nope"}
    # 先触发精确失败 warn
    for _ in range(3):
        ctrl.after_call("bash", args, "Error: fail", True)
    # 一次成功的变更操作（不同命令）
    ctrl.after_call("bash", {"command": "echo ok"}, "ok", False)
    # 再失败，exact 计数应已清零，不再是 warn
    dec = ctrl.after_call("bash", args, "Error: fail", True)
    assert dec.count == 1  # 视为第 1 次，非延续


# ── GuardrailController — reset_turn ────────────────────────────────

def test_reset_turn_clears_state(ctrl):
    args = {"command": "cat /nope"}
    for _ in range(4):
        ctrl.after_call("bash", args, "Error: fail", True)
    ctrl.reset_turn()
...
def test_executor_block_skips_execution():
    ...
    for _ in range(3):
        executor._guardrail.after_call("fake", args, "same", False)
...
def test_executor_begin_turn_resets_guardrail():
    """begin_turn 重置守护栏；之后状态从零开始。"""
    tool = _FakeTool("Error: fail", ok=False)
    ctrl = GuardrailController(interactive=True)
    reg = _FakeRegistry(tool)
    executor = ToolExecutor(reg, guardrail=ctrl)
